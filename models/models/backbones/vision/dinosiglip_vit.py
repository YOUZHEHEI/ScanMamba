...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        if self.image_resize_strategy == "resize-naive":
            ...
            siglip_transform = Compose([
                Resize(target_size, interpolation=sl_resize_transform.interpolation),
                *default_siglip_transform.transforms[1:]
            ])
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if siglip_input.dtype != target_dtype:
            siglip_input = siglip_input.to(dtype=target_dtype)
        if siglip_input.device != device:
            siglip_input = siglip_input.to(device)
        siglip_patches = self.siglip_featurizer(siglip_input)

        if dino_patches.dtype != siglip_patches.dtype:
            dino_patches = dino_patches.to(dtype=target_dtype)
            siglip_patches = siglip_patches.to(dtype=target_dtype)
